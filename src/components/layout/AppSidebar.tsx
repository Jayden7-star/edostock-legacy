...
const AppSidebar = ({ collapsed, onToggle, mobileOpen, onMobileClose }: AppSidebarProps) => {
  ...
  const renderNav = (isCollapsed: boolean, onNavClick?: () => void) => (
    <>
      {navItems.map((item) => {
        if (item.children) {
  ...
  return (
    <>
      ...
      <motion.aside
        ...
        className="hidden md:flex fixed left-0 top-0 h-screen bg-sidebar border-r border-sidebar-border z-40 flex-col"
      >
        {/* ロゴ */}
      ...
      <AnimatePresence>
        {mobileOpen && (
          <>
            ...
            />
            <motion.aside
              className="fixed left-0 top-0 h-screen w-[260px] z-50 bg-sidebar border-r border-sidebar-border flex flex-col md:hidden"
              initial={{ x: -260 }}
              animate={{ x: 0 }}
              exit={{ x: -260 }}
              transition={{ duration: 0.3, ease: "easeInOut" }}
            >
              ...
              <div className="h-16 flex items-center justify-between px-4 border-b border-sidebar-border">
                <Link to="/" onClick={onMobileClose} className="flex items-center gap-3 min-w-0 cursor-pointer">
                  <div className="w-9 h-9 rounded-lg bg-primary/15 border border-primary/25 flex items-center justify-center flex-shrink-0">
                    <span className="text-sm font-bold edo-gradient-text">江</span>
                  </div>
